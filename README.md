<h1 align="center"> Olá, meu nome é Rodrigo Diniz! 👋 </h1>

<p align="center">
  Estudante de Análise e Desenvolvimento de Sistemas pelo Instituto Federal de São Paulo (IFSP), em transição para o mercado de Tecnologia com foco em Dados e CRM.
</p>

### Sobre mim

- 🔭 **Atualmente trabalhando em:** Conciliando minha atuação no setor de faturamento hospitalar com o desenvolvimento de algoritmos e estruturas de dados.
- 🌱 **Atualmente aprendendo:** Aprofundando conhecimentos em Banco de Dados, Data Science, Data Analytics e CRM.
- 👯 **Aberto a colaborar em:** Projetos open-source voltados para análise de dados e otimização de processos.
- 🤔 **Buscando ajuda para:** Ingressar na minha primeira oportunidade de estágio em TI.
- 💬 **Pergunte-me sobre:** Programação em C, estruturação de dados (falo horas sobre BucketSort!) e até sobre montagem de hardware de PCs.
- 📫 **Como me encontrar:** [LinkedIn](https://www.linkedin.com/in/rodrigo-diniz-19a997193/) ou pelo e-mail *rodrigodinizsotero10@gmail.com*.
- 😄 **Pronomes:** Ele/Dele.
- ⚡ **Fato curioso:** Troco facilmente qualquer vídeo-aula por um bom artigo técnico escrito para estudar!

---

### 💻 Tecnologias e Ferramentas

<p align="left">
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/c/c-original.svg" height="40" alt="C logo"  />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/sqldeveloper/sqldeveloper-original.svg" height="40" alt="SQL logo"  />
  <img width="12" />
  <img src="https://cdn.jsdelivr.net/gh/devicons/devicon/icons/html5/html5-original.svg" height="40" alt="HTML5 logo"  />
</p>

---

### 📊 Estatísticas do GitHub

<div>
  <a href="https://github.com/rodrigodinz">
  <img loading="lazy" height="180em" src="https://github-readme-stats.vercel.app/api/top-langs/?username=rodrigodiniz&layout=compact&langs_count=7&theme=dracula"/>
  <img loading="lazy" height="180em" src="https://github-readme-stats.vercel.app/api?username=rodrigodinz&show_icons=true&theme=dracula&include_all_commits=true&count_private=true"/>
</div>

![Snake animation](https://github.com/rodrigodinz/rodrigodinz/blob/output/github-contribution-grid-snake.svg)


